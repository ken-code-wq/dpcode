...
`,
  ...
  it("builds Electron framework symlink paths for the top-level app bundle", () => {
    assert.deepStrictEqual(buildMacUpdateZipSymlinkEntries("Synara Dev.app"), [
      "Synara Dev.app/Contents/Frameworks/Electron Framework.framework/Electron Framework",
      "Synara Dev.app/Contents/Frameworks/Electron Framework.framework/Helpers",
      "Synara Dev.app/Contents/Frameworks/Electron Framework.framework/Libraries",
      "Synara Dev.app/Contents/Frameworks/Electron Framework.framework/Resources",
      "Synara Dev.app/Contents/Frameworks/Electron Framework.framework/Versions/Current",
    ]);
  });
  ...
  it("resolves exactly one top-level .app from update zip entries", () => {
    assert.equal(
      resolveSingleTopLevelMacAppBundle([
        "__MACOSX/Synara Dev.app/Contents/Info.plist",
        "Synara Dev.app/Contents/Info.plist",
        "Synara Dev.app/Contents/MacOS/Synara Dev",
      ]),
      "Synara Dev.app",
    );
...
files:
  - url: Synara-Dev-0.1.4-arm64.zip
    sha512: oldzip
    size: 100
  - url: Synara-Dev-0.1.4-arm64.dmg
    sha512: olddmg
    size: 200
path: 'Synara-Dev-0.1.4-arm64.zip'
sha512: oldzip
releaseDate: '2026-06-07T12:00:00.000Z'
`;

    const updated = updateMacUpdateManifestZipEntry(manifest, "Synara-Dev-0.1.4-arm64.zip", {
      sha512: "newzip",
      size: 12345,
    });
...
releaseDate: '2026-06-07T12:00:00.000Z'
`,
          "Synara-Dev-0.1.4-arm64.zip",
          {
            sha512: "newzip",
            size: 12345,
          },
        ),
      /Could not update Synara-Dev-0.1.4-arm64.zip entry/,
    );
  });
...
files:
  - url: Synara-Dev-0.1.5-arm64.zip
    sha512: newzip
    size: 12345
path: Synara-Dev-0.1.5-arm64.zip
sha512: newzip
releaseDate: '2026-06-07T12:00:00.000Z'
`;
    ...
    assert.deepStrictEqual(
      validateMacUpdateManifestZipMetadata(manifest, "Synara-Dev-0.1.5-arm64.zip", metadata),
      {
    ...
    assert.deepStrictEqual(
      assertMacUpdateManifestZipMetadata(manifest, "Synara-Dev-0.1.5-arm64.zip", metadata),
      {
        manifestHasZipPath: true,
        manifestHasZipSha: true,
        manifestHasZipSize: true,
